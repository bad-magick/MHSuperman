...
namespace NetVars
{
	...
	CNTClass* CNTClass::Analyze( CNTClass *pParent, RecvTable *pTable, int offset, bool addtolist )
	{
		...
		for ( int i = 0; i<m_pTable->GetNumProps(); i++ )
		{
			...
			if ( var->Analyze( this, m_pTable->GetProp(i) ) ) 
	...
	bool CNTVar::Analyze( CNTClass *pClass, RecvProp *pProp )
	{
		...
		else if ( pSubTable )
		{
			...
			if ( numprops>=1 && !strcmp( pSubTable->GetProp(0)->GetName(), "000" ) ||
				numprops>=2 && !strcmp( pSubTable->GetProp(1)->GetName(), "000" ) )
			{
	...
	void CNTVar::ExportRaw( std::ostream &out, int depth )
	{
		...
		std::string type;
		RawTypeName( type );
		out << "Name = " << m_pProp->GetName() << " \tOffset = " << std::hex << "0x" << m_pProp->GetOffset() << " \tType = " << type << "\n" << std::dec;
	}
