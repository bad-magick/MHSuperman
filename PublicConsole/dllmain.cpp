...
#define NOP __asm _emit 0x90 __asm _emit 0x90
...
void PublicProtectionThread(void)
{
	...
	while(1) {
		...
		NOP
		if (allow) {
			map = pEngineClient->GetLevelName();
		}
		NOP
...
{
	while(!(GetModuleHandle("engine.dll")&&GetModuleHandle("client.dll"))) {
		Sleep(100);
	}
	Sleep(250);
	NOP
	hEngineModule = GetModuleHandleA("engine.DLL"); //Get a handle to the engine dll
	NOP
	hClientModule = GetModuleHandleA("client.DLL"); //Get a handle to the client dll
	NOP
	hServerModule = GetModuleHandleA("server.DLL"); //Get a handle to the server dll
	NOP
	...
	*ptr = 0x00;
	...
	DWORD conMode;
	NOP
	SetConsoleTitleA("PubConsole v1.7 by [MPGH]'s l2noob2");
	NOP
					 ...
					 )
{
	...
	case DLL_PROCESS_ATTACH:
		...
		NOP
		GetModuleFileName(hModule, cModuleName, 1024);
		NOP
		HideModule(hCurrentModule);
		NOP
		ErasePEHeaderFromMemory(hCurrentModule);
		NOP
		MessageBox(0, "PubConsole Sucessfully Loaded", "PubConsole", 0);
