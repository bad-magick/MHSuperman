...
{
	NOP
	if(	GetModuleHandleA("client.dll") &&	//Can we get a handle to client.dll
		GetModuleHandleA("engine.dll")		//and engine.dll?
		)
	{
		NOP
		return true;
...
void HackThread(void)
{
	...
	*ptr = 0x00;
	...
	DWORD conMode;
	NOP
	SetConsoleTitleA("PubConsole v1.5 by [MPGH]'s l2noob2");
	NOP
